...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--password",
        type=str,
        help="(optional) password for the user",
        default="password",
    )
    ...
    parser.add_argument(
        ...
        "--passthrough",
        type=str,
        help="(optional) passthrough arguments to add to the docker build",
    )

    args = parser.parse_args()
    print("building docker image named ", args.image)
    cmd = (
        "docker build --build-arg USER_NAME=%(user_name)s --build-arg USER_PASSWORD=%(password)s --build-arg USER_ID=%(user_id)s --build-arg USER_GID=%(group_id)s"
        % {
            "user_name": user_name,
