...
class ThorEnv(Controller):
    ...
    def reset(
        ...
        scene_name = scene_name_or_num if type(scene_name_or_num) == str else "FloorPlan%d" % scene_name_or_num
    ...
    def restore_scene(self, object_poses, object_toggles, dirty_and_empty):
        ...
        if len(object_toggles) > 0:
            super().step({"action": "SetObjectToggles", "objectToggles": object_toggles})

        if dirty_and_empty:
            super().step({"action": "SetStateOfAllObjects", "StateChange": "CanBeDirty", "forceAction": True})
            super().step({"action": "SetStateOfAllObjects", "StateChange": "CanBeFilled", "forceAction": False})
        super().step({"action": "SetObjectPoses", "objectPoses": object_poses})
        ...
        if event.metadata["lastActionSuccess"]:
            ...
            if action["action"] == "ToggleObjectOn" and "Faucet" in action["objectId"]:
                ...
                self.cleaned_objects = (
                    self.cleaned_objects | set(cleaned_object_ids) if cleaned_object_ids is not None else set()
                )
            # heat
            if action["action"] == "ToggleObjectOn" and "Microwave" in action["objectId"]:
                ...
                self.heated_objects = (
                    self.heated_objects | set(heated_object_ids) if heated_object_ids is not None else set()
                )
            # cool
            if action["action"] == "CloseObject" and "Fridge" in action["objectId"]:
                ...
                self.cooled_objects = (
                    self.cooled_objects | set(cooled_object_ids) if cooled_object_ids is not None else set()
                )
    ...
    def smooth_move_ahead(self, action, render_settings=None):
        ...
        events = []
        for _xx in range(smoothing_factor - 1):
    ...
    def smooth_rotate(self, action, render_settings=None):
        ...
        end_rotation = start_rotation - 90 if action["action"] == "RotateLeft" else start_rotation + 90
    ...
    def to_thor_api_exec(self, action, object_id="", smooth_nav=False):
        ...
        if "RotateLeft" in action:
            ...
            action = {"action": "RotateRight", "forceAction": True}
            event = self.step(action, smooth_nav=smooth_nav)
        elif "MoveAhead" in action:
            action = {"action": "MoveAhead", "forceAction": True}
            event = self.step(action, smooth_nav=smooth_nav)
        elif "LookUp" in action:
            action = {"action": "LookUp", "forceAction": True}
            event = self.step(action, smooth_nav=smooth_nav)
        elif "LookDown" in action:
            action = {"action": "LookDown", "forceAction": True}
            event = self.step(action, smooth_nav=smooth_nav)
        elif "OpenObject" in action:
            action = {"action": "OpenObject", "objectId": object_id, "moveMagnitude": 1.0}
            ...
            action = {"action": "PickupObject", "objectId": object_id}
            event = self.step(action)
            ...
            action = {"action": "ToggleObjectOn", "objectId": object_id}
            event = self.step(action)

        elif "ToggleObjectOff" in action:
            action = {"action": "ToggleObjectOff", "objectId": object_id}
            event = self.step(action)
            ...
            action = {"action": "SliceObject", "objectId": object_id}
            event = self.step(action)
        else:
