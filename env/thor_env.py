...
from ..gen.utils import game_util, image_util
...
class ThorEnv(Controller):
    ...
    def step(self, action, smooth_nav=False):
        """overrides ai2thor.controller.Controller.step() for smooth navigation
        and goal_condition updates."""
        if smooth_nav:
    ...
    def check_clean(self, object_id):
        ...
        In this case, we need to execute a `CleanAction` in the
        simulator on every object in the corresponding basin. This is to
        clean everything in the sink rather than just things touching
        the stream.
        """
        event = self.last_event
