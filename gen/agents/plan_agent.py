...
class PlanAgent(AgentBase):
    ...
    def execute_plan(self):
        ...
        if constants.OPEN_LOOP:
            ...
            if plan[0]["action"] == "End":
                ...
                print(
                    "Planning failed. Possibly because the goal was already satisfied"
                )
            ...
            for idx, plan_action in enumerate(plan):
                ...
                step_count += 1
                if (
                    self.controller_agent.current_frame_count
            ...
            if constants.USE_DETERMINISTIC_CONTROLLER:
                ...
                rotations = 0
                while rotations < 4 and (
                    plan_action is None or plan_action["action"] == "End"
                ):
            ...
            while not (plan_action is None or plan_action["action"] == "End"):
                self.controller_agent.step(
                    self.game_state.get_plan_action(plan_action), executing_plan=True
                )
                ...
                if plan[0]["action"] == "End":
                    break
                if (
                    step_count >= constants.MAX_PLANNER_STEP_COUNT
                    or self.controller_agent.current_frame_count
                    >= constants.MAX_EPISODE_LENGTH
                ):
                    # Too many steps, plan may be looping.
                    break
                if len(plan) > 1 and any(
                    [plan == past_plan for past_plan in past_plans]
                ):
                    plan_action = plan[0]
                    self.controller_agent.step(
                        self.game_state.get_plan_action(plan_action),
                        executing_plan=True,
                    )
                    step_count += 1
                    plan = plan[1:]
    ...
    def save_plan(self, plan, idx=0):
        ...
        constants.data_dict["plan"]["high_pddl"].append(
            {
                "high_idx": len(constants.data_dict["plan"]["high_pddl"]),
                "planner_action": plan_action,
                "discrete_action": game_util.get_discrete_hl_action(plan, idx),
            }
        )
        constants.data_dict["template"]["high_descs"].append(
            game_util.get_templated_action_str(plan, idx)
        )
