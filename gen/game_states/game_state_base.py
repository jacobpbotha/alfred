...
class GameStateBase(object):
    ...
    def reset(
        ...
        new_scene = (
            self.gt_graph is None
            or self.gt_graph.scene_id is None
            or self.gt_graph.scene_id == scene_num
        )
        ...
        if max_num_repeats is None:
            ...
            self.env.step(
                dict(
                    ...
                    forceVisible=False,
                    numRepeats=[
                        {"objectType": o, "count": c} for o, c in objs["repeat"]
                    ]
                    if objs is not None and "repeat" in objs
                    else None,
                    minFreePerReceptacleType=free_per_receptacle
                    if objs is not None
                    else None,
                )
            )
            ...
            if objs is not None and ("seton" in objs and len(objs["seton"]) > 0):
                self.env.step(
                    dict(
                        ...
                        objectToggles=[
                            {"objectType": o, "isOn": v} for o, v in objs["seton"]
                        ],
                    )
                )

        self.gt_graph = graph_obj.Graph(
            use_gt=True, construct_graph=True, scene_id=self.scene_num
        )
        ...
        for idx in range(len(objects)):
            if objects[idx]["parentReceptacles"] is None:
                ...
                for jdx in range(len(objects)):
                    if (
                        objects[jdx]["receptacleObjectIds"] is not None
                        and objects[idx]["objectId"]
                        in objects[jdx]["receptacleObjectIds"]
                    ):
                        objects[idx]["parentReceptacles"].append(
    ...
    def store_image_name(self, name):
        constants.data_dict["images"].append(
            {
                "high_idx": game_util.get_last_hl_action_index(),
    ...
    def get_ll_discrete_action(self, action):
        ...
        if "TeleportFull" in a_type:
            print(
                "WARNING: Low-level actions should not use TeleportFull. Instead use LookUp or LookDown"
            )
            look_str = "Look"
            if action["horizon"] == 0:
                look_str += "Ahead"
            ...
            discrete_action["action"] = "MoveAhead_%d" % int(
                constants.AGENT_STEP_SIZE * 100
            )
    ...
    def get_bbox_of_obj(self, object_id):
        instance_detections2D = (
            ...
            if self.env.last_event.instance_detections2D != None
            else []
        )
        ...
        if object_id in instance_detections2D:
            ...
            return list(
                [int(np_box[0]), int(np_box[1]), int(np_box[2]), int(np_box[3])]
            )  # weird json formatting issue
        else:
            raise Exception("No bounding boxes on screen for %s" % object_id)

    def get_point_of_obj(
        self, object_id, centroid_type="box_center", visualize_debug=False
    ):
    ...
    def get_some_visible_obj_of_name(self, name):
        objects = [
            ...
            if obj["objectType"] == name and obj["visible"]
        ]
        if len(objects) == 0:
            ...
            if (
                ...
                if constants.RECORD_VIDEO_IMAGES:
                    ...
                    if "Teleport" in action["action"]:
                        ...
                        start_horizon = self.env.last_event.metadata["agent"][
                            "cameraHorizon"
                        ]
                        start_rotation = rotation["y"]
                        if (
                            np.abs(action["x"] - position["x"]) > 0.001
                            or np.abs(action["z"] - position["z"]) > 0.001
                        ):
                            # Movement
                            for xx in np.arange(0.1, 1, 0.1):
                                new_action = copy.deepcopy(action)
                                new_action["x"] = np.round(
                                    position["x"] * (1 - xx) + action["x"] * xx, 5
                                )
                                new_action["z"] = np.round(
                                    position["z"] * (1 - xx) + action["z"] * xx, 5
                                )
                                new_action["rotation"] = start_rotation
                                new_action["horizon"] = start_horizon
                                self.event = self.env.step(new_action)
                                cv2.imwrite(
                                    constants.save_path + "/%09d.png" % im_ind,
                                    self.event.frame[:, :, ::-1],
                                )
                                game_util.store_image_name(
                                    "%09d.png" % im_ind
                                )  # eww... seriously need to clean this up
                                im_ind += 1
                        if (
                            np.abs(
                                action["horizon"]
                                - self.env.last_event.metadata["agent"]["cameraHorizon"]
                            )
                            > 0.001
                        ):
                            ...
                            for xx in np.arange(0.1, 1, 0.1):
                                new_action = copy.deepcopy(action)
                                new_action["horizon"] = np.round(
                                    start_horizon * (1 - xx) + end_horizon * xx, 3
                                )
                                new_action["rotation"] = start_rotation
                                self.event = self.env.step(new_action)
                                cv2.imwrite(
                        ...
                        close_action = dict(
                            action=action["action"], objectId=action["objectId"]
                        )
                        ...
                        if (
                            parent_recep is not None
                            and parent_recep["objectType"]
                            in constants.OPENABLE_CLASS_SET
                        ):
                            self.open_recep(parent_recep)  # stores LL action

                        # close/open the object if needed
                        pickup_obj = game_util.get_object(
                            action["objectId"], self.env.last_event.metadata
                        )
                        if (
                            pickup_obj["objectType"]
                            in constants.FORCED_OPEN_STATE_ON_PICKUP
                        ):
                            if (
                                pickup_obj["isOpen"]
                                != constants.FORCED_OPEN_STATE_ON_PICKUP[
                                    pickup_obj["objectType"]
                                ]
                            ):
                        ...
                        if parent_recep is not None:
                            parent_recep = game_util.get_object(
                                parent_recep["objectId"], self.env.last_event.metadata
                            )
                        ...
                        if len(self.env.last_event.metadata["inventoryObjects"]) > 0:
                            inv_obj = self.env.last_event.metadata["inventoryObjects"][
                                0
                            ]["objectId"]
                        else:
                            raise RuntimeError(
                                "Taking 'PutObject' action with no held inventory object"
                            )
                        action["objectId"] = inv_obj

                        # open the receptacle if needed
                        parent_recep = game_util.get_object(
                            action["receptacleObjectId"], self.env.last_event.metadata
                        )
                        if (
                            parent_recep is not None
                            and parent_recep["objectType"]
                        ...
                        # Open the parent receptacle of the movable receptacle target.
                            ...
                            parent_recep["objectType"]
                            in constants.MOVABLE_RECEPTACLES_SET
                        ):
                            movable_parent_recep_ids = parent_recep["parentReceptacles"]
                            if (
                                movable_parent_recep_ids is not None
                                and len(movable_parent_recep_ids) > 0
                            ):
                                print(
                                    parent_recep["objectId"], movable_parent_recep_ids
                                )  # DEBUG
                                movable_parent_recep = game_util.get_object(
                                    movable_parent_recep_ids[0],
                                    self.env.last_event.metadata,
                                )
                                if (
                                    movable_parent_recep["objectType"]
                                    in constants.OPENABLE_CLASS_SET
                                ):
                                    self.open_recep(
                                        movable_parent_recep
                                    )  # stores LL action
                        ...
                        sink_obj_id = self.get_some_visible_obj_of_name("SinkBasin")[
                            "objectId"
                        ]
                        ...
                        clean_action[
                            "objectId"
                        ] = game_util.get_obj_of_type_closest_to_obj(
                            "Faucet", inv_obj["objectId"], self.env.last_event.metadata
                        )[
                            "objectId"
                        ]
                        ...
                        self.event = self.env.step(
                            {k: clean_action[k] for k in ["action", "objectId"]}
                        )
                        ...
                        sink_basin_obj = game_util.get_obj_of_type_closest_to_obj(
                            "SinkBasin",
                            clean_action["objectId"],
                            self.env.last_event.metadata,
                        )
                        for in_sink_obj_id in sink_basin_obj["receptacleObjectIds"]:
                            if (
                                game_util.get_object(
                                    in_sink_obj_id, self.env.last_event.metadata
                                )["dirtyable"]
                                and game_util.get_object(
                                    in_sink_obj_id, self.env.last_event.metadata
                                )["isDirty"]
                            ):
                                self.event = self.env.step(
                        ...
                        self.save_act_image(close_action, dir=constants.BEFORE)
                        self.event = self.env.step(
                            {k: close_action[k] for k in ["action", "objectId"]}
                        )
                        ...
                        pickup_action = dict(
                            action="PickupObject", objectId=inv_obj["objectId"]
                        )
                        self.store_ll_action(pickup_action)
                        self.save_act_image(pickup_action, dir=constants.BEFORE)
                        self.event = self.env.step(pickup_action)
                        ...
                        # open the microwave
                        microwave_obj_id = self.get_some_visible_obj_of_name(
                            "Microwave"
                        )["objectId"]
                        microwave_obj = game_util.get_object(
                            microwave_obj_id, self.env.last_event.metadata
                        )
                        ...
                        self.event = self.env.step(
                            {k: heat_action[k] for k in ["action", "objectId"]}
                        )
                        ...
                        self.event = self.env.step(
                            {k: stop_action[k] for k in ["action", "objectId"]}
                        )
                        ...
                        # open the fridge
                        fridge_obj_id = self.get_some_visible_obj_of_name("Fridge")[
                            "objectId"
                        ]
                        fridge_obj = game_util.get_object(
                            fridge_obj_id, self.env.last_event.metadata
                        )
                        ...
                        cool_action = dict(
                            action="CloseObject", objectId=action["objectId"]
                        )
                        ...
                        toggle_obj = game_util.get_object(
                            action["objectId"], self.env.last_event.metadata
                        )
                        on_action["action"] = (
                            "ToggleObjectOff"
                            if toggle_obj["isToggled"]
                            else "ToggleObjectOn"
                        )
                ...
                point_dists = np.sum(
                    np.abs(self.gt_graph.points - np.array(new_pose)[:2]), axis=1
                )
    ...
    def correct_slice_id(self, object_id):
        ...
        if (
            ...
            and main_obj["objectType"] in constants.VAL_ACTION_OBJECTS["Sliceable"]
            and main_obj["isSliced"]
        ):
            ...
            while True:
                slice_obj_id = (
                    main_obj["objectId"] + "|" + slice_obj_type + "_%d" % sidx
                )
                slice_obj = game_util.get_object(
                    slice_obj_id, self.env.last_event.metadata
                )
    ...
    def open_recep(self, recep):
        if recep["openable"] and not recep["isOpen"]:
            open_action = dict(action="OpenObject", objectId=recep["objectId"])
    ...
    def close_recep(self, recep):
        if recep["openable"] and recep["isOpen"]:
            close_action = dict(action="CloseObject", objectId=recep["objectId"])
            self.store_ll_action(close_action)
            self.save_act_image(close_action, dir=constants.BEFORE)
            self.event = self.env.step(close_action)
            self.save_act_image(close_action, dir=constants.AFTER)
    ...
    def get_parent_receps(self, object_id):
        ...
        parent_receps = [
            game_util.get_object(recep, self.env.last_event.metadata)
            for recep in obj["parentReceptacles"]
        ]
        parent_receps = [
            recep
            for recep in parent_receps
            if recep["objectType"] in constants.OPENABLE_CLASS_SET
        ]
    ...
    def check_obj_visibility(self, action, min_pixels=constants.MIN_VISIBLE_PIXELS):
        for o in self.env.last_event.metadata["objects"]:
            if (
                o["objectId"] == action["objectId"]
                and "Sliced" not in action["objectId"]
            ):  # NOTE: ignore slice objects for this check
                instance_masks = self.env.last_event.instance_masks
                total_obj_pixels = np.sum(
                    instance_masks[o["objectId"]]
                    if o["objectId"] in instance_masks
                    else [0]
                )
                # print("\tNum. Object Pixels: " + str(total_obj_pixels))
                if not o["visible"] or (total_obj_pixels < min_pixels):
                    raise Exception(
                        "Pickup item not visible! Visible property: "
                        + str(o["visible"])
                        + "; visible pixes: "
                        + str(total_obj_pixels)
                    )
    ...
    def save_act_image(self, action, dir=constants.BEFORE):
        self.save_image(
            constants.SAVE_FRAME_BEFORE_AND_AFTER_COUNTS[action["action"]][dir]
        )
