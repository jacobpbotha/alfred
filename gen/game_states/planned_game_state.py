...
class PlannedGameState(GameStateBase, ABC):
    ...
    def __init__(
        self, env, seed=None, action_space=None, domain=None, domain_path=None
    ):
...
"""
        ...
        opened_receptacle_str = "\n        ".join(
            ["(opened %s)" % obj for obj in self.currently_opened_object_ids]
        )
        ...
        for key, val in self.in_receptacle_ids.items():
            ...
            for vv in val:
                ...
                if (
                    vv_cls == constants.OBJECTS[self.object_target]
                    or (
                        self.mrecep_target is not None
                        and vv_cls == constants.OBJECTS[self.mrecep_target]
                    )
                    or (len(self.knife_obj) > 0 and vv_cls in self.knife_obj)
                ):
                ...
                if (
                    key_cls not in constants.MOVABLE_RECEPTACLES_SET
                    and vv_cls == constants.OBJECTS[self.object_target]
                ):
                    was_in_receptacle_strs.append(
                        "(wasInReceptacle  %s %s)" % (vv, key)
                    )
        ...
        objects = set(
            {
                ...
                if (
                    ...
                    or (
                        self.mrecep_target is not None
                        and obj["objectType"] == constants.OBJECTS[self.mrecep_target]
                    )
                    or (
                        (
                            self.toggle_target is not None
                            and obj["objectType"]
                            == constants.OBJECTS[self.toggle_target]
                        )
                        or (
                            (
                                len(self.knife_obj) > 0
                                and (
                                    obj["objectType"] in self.knife_obj
                                    or obj["objectType"] in movable_recep_cls_with_knife
                                )
                            )
                        )
                    )
        ...
        receptacle_str = "\n        ".join(
            sorted([receptacle + " # receptacle" for receptacle in receptacles])
        )
        ...
        for obj, loc in self.object_to_point.items():
            ...
            if (
                obj_cls == constants.OBJECTS[self.object_target]
                or (
                    self.toggle_target is not None
                    and obj_cls == constants.OBJECTS[self.toggle_target]
                )
                or (len(self.knife_obj) > 0 and obj_cls in self.knife_obj)
                or (obj_cls in constants.MOVABLE_RECEPTACLES_SET)
            ):
        ...
        if constants.PRUNE_UNREACHABLE_POINTS:
            ...
            receptacle_type_str = "\n        ".join(
                [
                    ...
                    if object_dict[receptacle]["objectType"]
                    not in constants.OPENABLE_CLASS_SET
                    or receptacle in openable_objects
                ]
            )
        else:
            receptacle_type_str = "\n        ".join(
        ...
        receptacle_objects_str = "\n        ".join(
            [
                "(isReceptacleObject %s)" % (obj)
                for obj in objects
                if object_dict[obj]["objectType"] in constants.MOVABLE_RECEPTACLES_SET
            ]
        )
        ...
        if constants.PRUNE_UNREACHABLE_POINTS:
            ...
            openable_str = "\n        ".join(
                [
                    ...
                    if object_dict[receptacle]["objectType"]
                    in constants.OPENABLE_CLASS_SET
                    and receptacle in openable_objects
                ]
            )

        dists = []
        dist_points = list(
            locations | {(self.pose[0], self.pose[1], self.pose[2], self.pose[3])}
        )
        for dd, l_start in enumerate(dist_points[:-1]):
            for l_end in dist_points[dd + 1 :]:
                actions, path = self.gt_graph.get_shortest_path_unweighted(
                    l_start, l_end
                )
                # Should cost one more for the trouble of going there at all. Discourages waypoints.
                dist = len(actions) + 1
                dists.append(
                    "(= (distance loc|%d|%d|%d|%d loc|%d|%d|%d|%d) %d)"
                    % (
                        l_start[0],
                        l_start[1],
                        l_start[2],
        ...
        cleanable_str = "\n        ".join(
            [
                "(cleanable %s)" % obj
                for obj in objects
                if object_dict[obj]["objectType"]
                in constants.VAL_ACTION_OBJECTS["Cleanable"]
            ]
        )
        ...
        is_clean_str = "\n        ".join(
            (
                [
                    "(isClean %s)" % obj
                    for obj in self.cleaned_object_ids
                    if object_dict[obj]["objectType"]
        ...
        heatable_str = "\n        ".join(
            [
                ...
                for obj in objects
                if object_dict[obj]["objectType"]
                in constants.VAL_ACTION_OBJECTS["Heatable"]
            ]
        )
        ...
        is_hot_str = "\n        ".join(
            (
                [
                    "(isHot %s)" % obj
                    for obj in self.hot_object_ids
                    if object_dict[obj]["objectType"]
                    == constants.OBJECTS[self.object_target]
                ]
            )
        ...
        coolable_str = "\n        ".join(
            [
                ...
                for obj in objects
                if object_dict[obj]["objectType"]
                in constants.VAL_ACTION_OBJECTS["Coolable"]
            ]
        )
        ...
        toggleable_str = "\n        ".join(
            [
                ...
                if (
                    self.toggle_target is not None
                    and object_dict[obj]["objectType"]
                    == constants.OBJECTS[self.toggle_target]
                )
            ]
        )
        ...
        sliceable_str = "\n        ".join(
            [
                ...
                for obj in objects
                if (
                    object_dict[obj]["objectType"]
                    in constants.VAL_ACTION_OBJECTS["Sliceable"]
                )
            ]
        )
        ...
        is_cool_str = "\n        ".join(
            (
                [
                    "(isCool %s)" % obj
                    for obj in self.cool_object_ids
                    if object_dict[obj]["objectType"]
                    == constants.OBJECTS[self.object_target]
                ]
            )
            ...
            [
                ...
                if (
                    ...
                    and (
                        self.mrecep_target is not None
                        and object_dict[obj]["objectType"]
                        == constants.OBJECTS[self.mrecep_target]
                    )
                )
            ]
        )
        ...
        receptacle_nearest_point_strs = sorted(
            [
                ...
                if (
                    object_dict[obj_id]["objectType"] in constants.RECEPTACLES
                    and object_dict[obj_id]["objectType"]
                    not in constants.MOVABLE_RECEPTACLES_SET
                )
            ]
        )
...
""" % (
        ...
        with open(state_save_file, "w") as fid:
            fid.write(pddl_str)
            fid.flush()
        constants.data_dict["pddl_state"].append(
            "problem_%s.pddl" % pddl_state_next_idx
        )

        with open(
            "%s/planner/generated_problems/problem_%s.pddl"
    ...
    def get_next_plan_action(self, force_update=False):
        ...
        if constants.DEBUG:
            print(
                ...
                + "\n".join(
                    [
                        "%03d %s" % (ii, game_util.get_action_str(pl))
    ...
    def reset(self, seed=None, info=None, scene=None, objs=None):
        ...
        super(PlannedGameState, self).reset(
            self.scene_num,
            False,
            self.scene_seed,
            max_num_repeats,
