...
from .planned_game_state import PlannedGameState
...
class TaskGameState(PlannedGameState):
    ...
    def __init__(self, env, seed=None, action_space=None):
        ...
        super(TaskGameState, self).__init__(
            ...
            action_space,
            "put_task",
            "planner/domains/PutTaskExtended_domain.pddl",
        )
        self.task_target = None
        self.success = False

    def get_task_str(self):
        return game_util.get_task_str(
            self.object_target,
            self.parent_target,
            self.toggle_target,
            self.mrecep_target,
    ...
    def get_goal_pddl(self):
        ...
        goal_str = goal_str.format(
            obj=constants.OBJECTS[self.object_target]
            ...
            if self.mrecep_target is not None
            else "",
        )
        return goal_str
    ...
    def initialize_random_scene(self, scene=None):
        if self.problem_id is not None:
            ...
            if not constants.DEBUG and os.path.exists(
    ...
    def get_filter_crit(self, goal_type):
        ...
        def does_have_duplicates(x):
            num_instances = len(
                game_util.get_objects_of_type(
                    x["objectType"], self.env.last_event.metadata
                )
            )
        ...
        def does_any_recep_type_have_obj_of_type(r, ot):
            ...
            for recep in all_recep_of_type:
                ...
                if receptacle_obj_ids is not None:
                    objs_of_type = [
                        obj_id for obj_id in receptacle_obj_ids if ot in obj_id
                    ]
        ...
        def is_obj_type_in_recep(x, recep):
            obj_types_in_recep = [
                ...
                for o in self.env.last_event.metadata["objects"]
                if (
                    o["parentReceptacles"] is not None
                    and any(recep in r for r in o["parentReceptacles"])
                )
            ]
        ...
        def is_obj_off(x):
            return (
                x["objectType"] in constants.VAL_ACTION_OBJECTS["Toggleable"]
                and x["toggleable"]
                and (not x["isToggled"])
            )
        ...
        if goal_type == "init":
            return lambda o: is_obj_pickupable(o), lambda r: is_receptacle(r) and (
                can_recep_hold_obj(self.rand_chosen_object_class, r)
            ) and (not is_recep_full(r)) and (
                is_obj_in_recep(self.rand_chosen_object, r)
            )
        elif goal_type == "place_all_obj_type_into_recep":
            return lambda o: does_have_duplicates(
                o
            ), lambda r: does_recep_have_enough_space(r)
        elif goal_type == "pick_two_obj_and_place":
            return lambda o: does_have_atleast_n_duplicates(
                o, 2
            ), lambda r: does_recep_have_enough_space(r) and (
                not does_any_recep_type_have_obj_of_type(
                    r, self.rand_chosen_object_class
                )
            )
            ...
            return lambda o: is_obj_pickupable(o), lambda r: r
        elif goal_type == "pick_and_place_with_movable_recep":
            return lambda o: is_obj_pickupable(o) and (
                not is_movable_recep(o)
            ), lambda r: is_receptacle(r) and (not is_movable_recep(r)) and (
                not does_any_recep_type_have_obj_of_type(
                    r, self.rand_chosen_val_moveable_recep_class
                )
            )
        elif goal_type == "pick_heat_and_place_with_movable_recep":
            return lambda o: is_obj_pickupable(o) and (is_obj_prop(o, "Heatable")) and (
                not is_movable_recep(o)
            ), lambda r: is_receptacle(r) and (not is_movable_recep(r)) and (
                not is_of_type(r, "Microwave")
            ) and (
                not does_any_recep_type_have_obj_of_type(
                    r, self.rand_chosen_val_moveable_recep_class
                )
            ...
            return lambda o: is_obj_pickupable(o), lambda r: is_receptacle(r)
    ...
    def setup_problem(self, seed=None, info=None, scene=None, objs=None):
        ...
        receptacle_objects = [
            ...
            if (
                o["receptacle"]
                and o["objectType"] in constants.RECEPTACLES
        ...
        if len(pickupable_objects) == 0:
            print("Task Failed - %s" % constants.pddl_goal_type)
            raise Exception(
                "No pickupable objects related to the goal '%s'"
        ...
        if "movable_recep" in constants.pddl_goal_type:
            val_movable_receps = [
                o
                for o in self.env.last_event.metadata["objects"]
                if (o["objectType"] in constants.MOVABLE_RECEPTACLES_SET)
                and (
                    self.rand_chosen_object_class
                    in constants.VAL_RECEPTACLE_OBJECTS[o["objectType"]]
                )
            ]
            ...
            if objs is not None:
                val_movable_receps = [
                    o for o in val_movable_receps if o["objectType"] == objs["mrecep"]
                ]
        ...
        if constants.pddl_goal_type == "look_at_obj_in_light":
            ...
            if constants.FORCED_SAMPLING or objs is None:
                ...
                toggle_class_available = [
                    obj for obj in toggle_objects if obj["objectType"] == objs["toggle"]
                ]
            ...
            val_receptacle_objects_orig = [
                r
                for r in receptacle_objects
                if (
                    self.rand_chosen_val_moveable_recep_class
                    if self.mrecep_target != None
                    else self.rand_chosen_object_class
                )
        ...
        # pddl_param dict
        constants.data_dict["pddl_params"]["object_target"] = (
            constants.OBJECTS[self.object_target]
            if self.object_target is not None
            else ""
        )
        constants.data_dict["pddl_params"]["object_sliced"] = requires_slicing
        constants.data_dict["pddl_params"]["parent_target"] = (
            constants.OBJECTS[self.parent_target]
            if self.parent_target is not None
            else ""
        )
        constants.data_dict["pddl_params"]["toggle_target"] = (
            constants.OBJECTS[self.toggle_target]
            if self.toggle_target is not None
            else ""
        )
        constants.data_dict["pddl_params"]["mrecep_target"] = (
        ...
        if self.parent_target is None:
            ...
            objs = game_util.get_objects_of_type(
                constants.OBJECTS[self.object_target], self.env.last_event.metadata
            )
            available_receptacles = game_util.get_objects_of_type(
                constants.OBJECTS[self.parent_target], self.env.last_event.metadata
            )
            validity_check = len(objs) > 0 and len(
                [obj for obj in available_receptacles]
            )
        ...
        if perform_sanity_check:
            try:
                assert (
                    validity_check
                ), "Task does not seem valid according to scene metadata"
