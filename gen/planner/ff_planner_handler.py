...
def parse_action_arg(action_arg):
    ...
    action_arg = py_util.multireplace(
        action_arg,
        {
            "_minus_": "-",
            "-": "#",
...
def get_plan_from_file(args):
    ...
    try:
        ...
        if (
            "goal can be simplified to FALSE" in output_str
            or "won't get here: simplify, non logical" in output_str
        ):
    ...
    if constants.DEBUG:
        print(
            "planned %s in %.5f, plan length %d solver type %d"
...
def get_plan_async(args):
    ...
    filepath = "%s/planner/generated_problems/problem_%s.pddl" % (
        constants.LOG_FILE,
        problem_id,
    )
...
class PlanParser(object):
    ...
    def get_plan_from_file(self, domain_path, filepath):
        parsed_plans = self.process_pool.map(
            get_plan_from_file, zip([domain_path] * 3, [filepath] * 3, range(3, 6))
        )
        return self.find_best_plan(parsed_plans)
    ...
    def clean_plan(self, plan):
        ...
        for i in range(len(plan) - 1):
            if not (
                plan[i]["action"] == "GotoLocation"
                and plan[i + 1]["action"] == "GotoLocation"
            ):
    ...
    def find_best_plan(self, parsed_plans):
        ...
        if all([parsed_plan[0] == "timeout" for parsed_plan in parsed_plans]):
            ...
            parsed_plans = [
                self.clean_plan(parsed_plan)
                for parsed_plan in parsed_plans
                if parsed_plan[0] != "timeout"
            ]
        ...
        if constants.DEBUG:
            print(
                ...
                + "\n".join(
                    [
                        "%03d: %s" % (pp, game_util.get_action_str(pl))
...
if __name__ == "__main__":
    ...
    print(
        ...
        + "\n".join(
            [
                "%03d: %s" % (pp, game_util.get_action_str(pl))
                for pp, pl in enumerate(result_plan)
            ]
        )
    )
