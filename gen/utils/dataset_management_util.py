...
def load_successes_from_disk(
    succ_dir, succ_traj, prune_trials, target_count, cap_count=None, min_count=None
):
    tuple_counts = {}
        ...
        for d in dirs:
            if d.count("-") == 4:
                ...
                if prune_trials:
                    if deleted_all:
                        print(
                            "Removing trial-less parent dir '%s'"
                            % os.path.join(succ_dir, d)
                        )
                        shutil.rmtree(os.path.join(succ_dir, d))
                    else:
                        for _d in queue_for_delete:
                            print(
                                "Removing unfinished trial '%s'"
                                % os.path.join(succ_dir, d, _d)
                            )
    ...
    tuples_at_target_count = set(
        [t for t in tuple_counts if tuple_counts[t] >= target_count]
    )
...
def load_fails_from_disk(succ_dir, to_write=None):
    ...
    if to_write is not None:
        for goal, pickup, movable, receptacle, scene_num in to_write:
            with open(
                os.path.join(
                    fail_dir, "-".join([goal, pickup, movable, receptacle, scene_num])
                ),
                "w",
            ) as f:
                f.write("0")
